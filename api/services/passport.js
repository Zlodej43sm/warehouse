...
passport.connect = function (req, query, profile, next) {
  ...
  if (profileJSON = profile._json) {
    if (profileJSON.first_name) user.first_name = profileJSON.first_name;
    if (profileJSON.last_name) user.last_name = profileJSON.last_name;
  }
...
passport.deserializeUser(function (id, next) {
  User.findOne(id).populateAll().exec(
    function(err, user) {
      if (user.role !== 'SU' && _.some(user.partners, item => item.isDisabled)) {
        next(null, false);
      }
      next(err, user);
    }
