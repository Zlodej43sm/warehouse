...
module.exports = function expand(req, res) {
  ...
  Model
    ...
    .exec(function found(err, matchingRecord) {
      ...
      var relModelName  = relation.replace(/s$/, '');
      var relationModel = req._sails.models[relModelName];
      var isDatatable   = relationModel && relationModel.datatable
                        && req.param('callback') == 'datatable';

      if (isDatatable) datatable.sendOk(res, relationModel, matchingRecord[relation], req.user);
      else return res.ok(matchingRecord[relation]);
    });
};
